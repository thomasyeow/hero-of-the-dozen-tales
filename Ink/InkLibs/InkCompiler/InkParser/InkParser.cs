...
namespace Ink
{
    public partial class InkParser : StringParser
    {
        ...
        void OnStringParserError(string message, int index, int lineIndex, bool isWarning)
        {
            ...
            string fullMessage = _filename != null
                ? string.Format(warningType + " '{0}' line {1}: {2}", _filename, lineIndex + 1, message)
                : string.Format(warningType + " line {0}: {1}", lineIndex + 1, message);
