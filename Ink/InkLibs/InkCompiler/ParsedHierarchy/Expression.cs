...
namespace Ink.Parsed
{
    ...
    public class BinaryExpression : Expression
    {
        ...
        string NativeNameForOp(string opName)
        {
            return opName == "and"
                ? "&&"
                : opName == "or" ? "||" : opName == "mod" ? "%" : opName == "has" ? "?" : opName == "hasnt" ? "!?" : opName;
        }
    ...
    public class IncDecExpression : Expression
    {
        ...
        public override void ResolveReferences(Story context)
        {
            ...
            {
                Error("variable for " + incrementDecrementWord + " could not be found: '" + varIdentifier + "' after searching: " + descriptionOfScope);
            }
        ...
        public override string ToString()
        {
            return expression ? varIdentifier + (isInc ? " += " : " -= ") + expression.ToString() : varIdentifier + (isInc ? "++" : "--");
        }
    ...
    public class MultipleConditionExpression : Expression
    {
        public List<Expression> subExpressions
        {
            get
            {
                return content.Cast<Expression>().ToList();
            }
        }
