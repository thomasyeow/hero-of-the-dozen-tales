...
namespace Ink.Runtime
{
    ...
    public class InkList : Dictionary<InkListItem, int>
    {
        ...
        public InkList(string singleOriginListName, Story originStory)
        {
            ...
            ListDefinition def;
            origins = originStory.listDefinitions.TryListGetDefinition(singleOriginListName, out def)
                ? new List<ListDefinition> { def }
        ...
        public void AddItem(string itemName)
        {
            ...
            foreach (var origin in origins)
            {
                ...
                {
                    foundListDef = foundListDef != null
        ...
        {
            _originNames = initialOriginNames == null ? null : new List<string>(initialOriginNames);
        }
        ...
        {
            return otherList.Count == 0 ? false : Count == 0 ? true : maxItem.Value < otherList.minItem.Value;
        }
        ...
        {
            return Count > 0 ? new InkList(maxItem) : new InkList();
        }

        public InkList MinAsList()
        {
            return Count > 0 ? new InkList(minItem) : new InkList();
        }
        ...
        List<KeyValuePair<InkListItem, int>> orderedItems
        {
            get
            {
                ...
                ordered.Sort((x, y) =>
                {
                    ...
                    return x.Value == y.Value ? x.Key.originName.CompareTo(y.Key.originName) : x.Value.CompareTo(y.Value);
                });
                return ordered;
            }
        }
