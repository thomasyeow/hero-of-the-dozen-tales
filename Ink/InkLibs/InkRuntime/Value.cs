namespace Ink.Runtime
{
    ...
    public class BoolValue : Value<bool>
    {
        ...
        public override Value Cast(ValueType newType)
        {
            return newType == valueType
                ...
                ? new IntValue(value ? 1 : 0)
                : newType == ValueType.Float
                ? new FloatValue(value ? 1.0f : 0.0f)
                : newType == ValueType.String ? (Value)new StringValue(value ? "true" : "false") : throw BadCastException(newType);
        }
    ...
    public class IntValue : Value<int>
    {
        ...
        public override Value Cast(ValueType newType)
        {
            return newType == valueType
                ? this
                : newType == ValueType.Bool
                ? new BoolValue(value == 0 ? false : true)
                : newType == ValueType.Float
                ? new FloatValue(value)
                : newType == ValueType.String ? (Value)new StringValue("" + value) : throw BadCastException(newType);
        }
    }
    ...
    public class FloatValue : Value<float>
    {
        ...
        public override Value Cast(ValueType newType)
        {
            return newType == valueType
                ? this
                : newType == ValueType.Bool
                ? new BoolValue(value == 0.0f ? false : true)
                : newType == ValueType.Int
                ? new IntValue((int)value)
                : newType == ValueType.String
                ? (Value)new StringValue("" + value.ToString(System.Globalization.CultureInfo.InvariantCulture))
    ...
    public class StringValue : Value<string>
    {
        ...
        public override Value Cast(ValueType newType)
        {
            ...
            if (newType == ValueType.Int)
            {
                ...
                int parsedInt;
                return int.TryParse(value, out parsedInt) ? new IntValue(parsedInt) : (Value)null;
            }

            if (newType == ValueType.Float)
            {
                float parsedFloat;
                return float.TryParse(value, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out parsedFloat)
                    ? new FloatValue(parsedFloat)
                    : (Value)null;
            }
    ...
    public class DivertTargetValue : Value<Path>
    {
        ...
        public override Value Cast(ValueType newType)
        {
            return newType == valueType ? (Value)this : throw BadCastException(newType);
        }
    ...
    public class ListValue : Value<InkList>
    {
        ...
        public override Value Cast(ValueType newType)
        {
            if (newType == ValueType.Int)
            {
                ...
                return max.Key.isNull ? new IntValue(0) : (Value)new IntValue(max.Value);
            }

            else if (newType == ValueType.Float)
            {
                var max = value.maxItem;
                return max.Key.isNull ? new FloatValue(0.0f) : (Value)new FloatValue(max.Value);
            }

            else if (newType == ValueType.String)
            {
                var max = value.maxItem;
                return max.Key.isNull ? new StringValue("") : (Value)new StringValue(max.Key.ToString());
            }
