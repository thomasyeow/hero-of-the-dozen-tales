...
namespace Ink.Runtime
{
    ...
    public class Profiler
    {
        ...
        public void Step(CallStack callstack)
        {
            ...
            stepType = controlCommandStep ? controlCommandStep.commandType.ToString() + " CC" : currObj.GetType().Name;
        ...
        {
            return num > 5000
                ? string.Format("{0:N1} secs", num / 1000.0)
                : num > 1000
                ? string.Format("{0:N2} secs", num / 1000.0)
                : num > 100
                    ? string.Format("{0:N0} ms", num)
                    : num > 1 ? string.Format("{0:N1} ms", num) : num > 0.01 ? string.Format("{0:N3} ms", num) : string.Format("{0:N} ms", num);
        }
...
#pragma warning restore 0649
		...
		public IEnumerable<KeyValuePair<string, ProfileNode>> descendingOrderedNodes
        {
            get
            {
                return _nodes == null ? null : (IEnumerable<KeyValuePair<string, ProfileNode>>)_nodes.OrderByDescending(keyNode => keyNode.Value._totalMillisecs);
            }
        }
