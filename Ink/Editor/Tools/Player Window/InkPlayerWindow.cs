...
namespace Ink.UnityIntegration
{
    ...
    public class InkPlayerWindow : EditorWindow
    {
        ...
        public class FunctionPanelState : BaseStoryPanelState
        {
            ...
            public class FunctionParams
            {
                ...
                public class FunctionInput
                {
                    ...
                    {
                        inkVariableValue = !string.IsNullOrWhiteSpace(inkVariablePath) ? story.variablesState[inkVariablePath] : null;
                    }
        ...
        {
            return StringContains(contentString, InkPlayerWindowState.Instance.storyPanelState.searchString, StringComparison.OrdinalIgnoreCase);
        }
        ...
        static bool ShouldShowContent(InkHistoryContentItem content, DisplayOptions.VisibilityOptions visibilityOpts)
        {
            switch (content.contentType)
            {
                case InkHistoryContentItem.ContentType.PresentedContent:
                    {
                        return (visibilityOpts & DisplayOptions.VisibilityOptions.Content) != 0
                            ? content.content.Length != 0 || (visibilityOpts & DisplayOptions.VisibilityOptions.EmptyEntries) != 0
                            : false;
                    }
        ...
        void DisplayStoryBody()
        {
            ...
            for (int i = 0; i < validHistory.Count; i++)
            {
                ...
                if (panelTop <= endY && panelBottom >= y)
                {
                    ...
                    GUIStyle lineStyle = selectedLine == content
                        ? historyItemBGStyleSelected.guiStyle
                        : i % 2 == 0 ? historyItemBGStyleDark.guiStyle : historyItemBGStyleLight.guiStyle;
        ...
        void DisplayProfileDataNode(ProfileNode node)
        {
            ...
            if (key == null)
            {
                key = node == _profilerResultRootNode ? "TOTAL" : "?";
            }
        ...
        {
            return toCheck.Length == 0 ? false : str.IndexOf(toCheck, comp) >= 0;
        }
